...
az network vpn-site create --name onprem -g vwan-microhack-hub-rg --ip-address $vnetgwtunnelip1 --virtual-wan microhack-vwan --location northeurope --device-model VNETGW --device-vendor Azure --asn $vnetgwasn --bgp-peering-address $vnetgwbgpip1 --link-speed-in-mbps 100 --with-link true

echo "# VWAN: Create connection - remote site link to hub gw"
az network vpn-gateway connection add --gateway-name microhack-we-hub-vng --name onprem-connection --vpn-site-link onprem -g vwan-microhack-hub-rg --shared-key $sharedkey --enable-bgp true --no-wait
