...
az network vpn-gateway connection create --gateway-name microhack-we-hub-vng --name onprem-connection  -g vwan-microhack-hub-rg --remote-vpn-site onprem --shared-key $sharedkey --enable-bgp true
az network vpn-gateway connection vpn-site-link-conn add --name on-prem-link-conn --connection-name onprem-connection --vpn-site-link onprem -g vwan-microhack-hub-rg --gateway-name --name microhack-we-hub-vng
