...
az network vnet-gateway list-bgp-peer-status -n vnet-gw-onprem-1 -g vwan-microhack-spoke-rg --output table

echo "# VNETGW: Display routes advertised from onprem gw to hub"
az network vnet-gateway list-advertised-routes -n vnet-gw-onprem-1 -g vwan-microhack-spoke-rg --peer $hubgwbgpaddress --output table

echo "# VNETGW: Display routes learned by onprem gw from hub"
az network vnet-gateway list-learned-routes -n vnet-gw-onprem-1 -g vwan-microhack-spoke-rg --output table
