...
sharedkey="m1cr0hack"
customrt=$(az network vhub route-table show --name custom-rt --resource-group vwan-microhack-hub-rg --vhub-name microhack-we-hub --query "id" --output tsv)
echo "Custom Routetable" 
...
az network local-gateway create -g vwan-microhack-spoke-rg -n lng --gateway-ip-address $hubgwtunneladdress --location swedencentral --asn $hubgwasn --bgp-peering-address $hubgwbgpaddress
